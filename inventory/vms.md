...
| 100 | truenas | `rb1-pve` | `rb2-pve` (planned pivot target) | High | Backup workflows recently healthy (2026-02-13), VM-level rollback artifact available | Planned migrate/restore to `rb2` before `rb1` baremetal conversion | `vmbr0` LAN (`192.168.5.100/22`) + storage dependencies | 2026-02-14 23:10 EST (running) | 2 | Pivot plan: make `rb2` storage-primary so `rb1` can move to Fedora baremetal |
